import { getPositiveIndexFromNegativeIndex } from "@/utils";
...
function calcCheckSumRol(bytes: number[]): number {
...
function calcCheckXor(bytes: number[]): number {
    console.assert(bytes && bytes.length > 0);
...
function checkSysexAndReplaceCheckSum(data: string[]): string[] {
    console.log("Checking the checksum...");
    ...
    if (!parsed || !parsed.isNeededCheckSum) {
        console.info("No need to check the checksum.");
        return data;
    }
    ...
    newBytes[getPositiveIndexFromNegativeIndex(newBytes, -2)] = parsed.calculatedCheckSum;

    // Convert the byte array to a hex string array with Ox prefix and always 2 digits and uppercase.
    const finalData = newBytes.map((e) => `0x${`0${e.toString(16)}`.slice(-2).toUpperCase()}`);
    console.info("Checksum is checked and replaced. New checksum is", finalData);

    return finalData;
}

export { parseSysEx, calcCheckSumRol, calcCheckSum, calcCheckXor, bytesToHex, checkSysexAndReplaceCheckSum };
